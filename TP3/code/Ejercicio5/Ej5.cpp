...
int main(int argc, char *argv[])
{
  ...
  MPI_Gatherv(local_primes.data(), local_size, MPI_LONG_LONG, global_primes.data(), all_sizes.data(), displacements.data(), MPI_LONG_LONG, 0, MPI_COMM_WORLD);

  // Proceso 0 ordena y muestra los resultados
  if (world_rank == 0)
  {
    timeval time1, time2;
    ...
    for (int i = std::max(0, int(global_primes.size()) - 10); i < global_primes.size(); ++i)
    {
      std::cout << global_primes[i] << " ";
    }
    std::cout << std::endl;

    std::cout << "Cantidad total de números primos menores que " << N << ": " << global_primes.size() << std::endl;
